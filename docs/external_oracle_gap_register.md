...
| --- | --- | --- | --- | --- | --- |
| ORACLE-001 | RFC 5225 RTP/UDP/IP, UDP/IP, ESP/IP, and IP-only | Closed only for mandatory bidirectional deterministic three-packet IPv4, small-CID-0, no-IP-options flows against the pinned rohc-lib; IPv6, other CID forms, IP options, and extension-header chains are not externally validated. | Keep the pinned oracle and strict corpus adapter in the required CTest set. | Mandatory | `interop_rfc5225_rohccxx_to_rohclib` and `interop_rfc5225_rohclib_to_rohccxx` pass. |
...
| ORACLE-004 | Every formal RFC 5225 CO packet family and discriminator variant | Mandatory external PT-0 coverage is bidirectional for UDP/IP, ESP/IP, and IP-only. The pinned rohc-lib RTP compressor stays in IR and its RTP decompressor rejects non-repair CO in an explicit TODO path. Live rohccxx CO-COMMON/CO-REPAIR conflicts with assisting-layer `0xFA/0xFB`. | Oracle able to parse `rfc5225_co_corpus` v4, including `crc_data`, `control_crc_data`, and generated ROHC bytes | Partial mandatory PT-0 coverage; remaining variants explicit and optional | `interop_rfc5225_co_rohccxx_to_rohclib` and `interop_rfc5225_co_rohclib_to_rohccxx` pass; a future oracle closes RTP and FA/FB gaps. |
