...
constexpr size_t Capacity=2048, CorpusPackets=65536, Establish=3, MinOps=100000;
...
static void validate_manifest(const std::string&m){auto s=slurp(m);for(auto*x:{"\"release\": \"0.4.1\"","\"protocol_revision\": \"v0.4.1-development-baseline-r1\"","\"seed_hex\": \"0x524f484343585832\"","\"packets_per_workload\": 65536","\"warmup_operations_per_treatment\": 10000","\"minimum_measured_operations\": 100000","\"minimum_measured_seconds\": 1.0","\"iterations\": 5","not a formal, marketing, absolute-performance, or cross-version benchmark"})require(s.find(x)!=std::string::npos,std::string("manifest: ")+x);require(s.find("[0, 20, 160, 1200]")!=std::string::npos,"payloads");require(s.find("[1, 4, 16]")!=std::string::npos,"flows");}
static void timing_identity(const char*self,const std::string&m){require(cmd("uname -m")=="x86_64","arch");require(cmd("/usr/bin/g++-13 -dumpfullversion")=="13.3.0","compiler");auto resolved=slurp(root+"/harness-build/experiment-resolved-v0.4.1.txt");for(auto p:{m,std::string(self),root+"/harness-src/scientific_runner.cpp",root+"/harness-build/effective-flags.txt",root+"/incoming/rohccxx-0.4.1-source-with-submodules.tar.gz",root+"/build-release/src/librohccxx.so.0.4.1",root+"/rohclib-build-normalized/src/.libs/librohc.so.3.0.0"})require(resolved.find(hash(p)+"  "+p)!=std::string::npos,"identity "+p);}
...
int main(int argc,char**argv)
{
 try {
  ...
  root=runroot;validate_manifest(manifest);if(mode!="correctness")timing_identity(argv[0],manifest);
