...
if len(cal) != 48 * 7 or len(rows) != 48 * 7 * 21:
    raise SystemExit(f"observation count mismatch calibration={len(cal)} final={len(rows)}")
for r in cal:
    if r['gate_status'] != 'CALIBRATION_NON_REPORTABLE' or int(r['operations']) < 1000000 or int(r['elapsed_ns']) < 4000000000 or int(r['guards']) or int(r['incorrect']): raise SystemExit('invalid calibration row')
for r in rows:
...
for key, values in sorted(groups.items()):
    if len(values) != 21: raise SystemExit(f'missing observation {key}')
...
for key in sorted({(r['profile'],r['payload'],r['flows'],r['implementation']) for r in rows if r['operation'] in ('steady_compress','control')}):
    ...
    compression.append(dict(zip(('profile','payload','flows','implementation'),key), compressed_bytes_median=statistics.median(int(r['output_bytes']) for r in selected), compression_ratio=statistics.median(ratios)))
